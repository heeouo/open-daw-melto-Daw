...
export default function InstrumentPlayer() {
  ...
  return (
    <div className="p-4 space-y-[4px]">
      <div className="flex space-x-[10px]">
        <button onClick={() => setInstrument('piano')} className={instrument==='piano'? 'px-[20px] py-[10px] bg-[#b8f5f4] text-white rounded':'px-[20px] py-[10px] bg-[#fcfcf7] rounded'}>Piano</button>
      ...
      {instrument === 'piano' && (
        <div className="flex items-center space-x-[5px]">
          ...
          <button onClick={() => setOctave(o => Math.min(7, o+1))} disabled={octave>=7} className="px-[20px] py-[10px] bg-[#fcfcf7] rounded">Octave Up</button>
        </div>
      )}
